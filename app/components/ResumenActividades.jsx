...
export default function ResumenActividades({ userId }) {
  ...
  const diffTime = (startIso, endIso) => {
  ...
  const fetchActivities = async () => {
    ...
    data.forEach((act) => {
      ...
      if (act.event_type === "clock_in") {
        lastClockIn = act.event_time;
        events.push({
          id: `in-${act.id}`,
          type: "Entrada",
          ...base,
      ...
      if (act.event_type === "clock_out") {
        events.push({
          id: `out-${act.id}`,
          type: "Salida",
          ...base,
          duration: diffTime(lastClockIn, act.event_time),
        });
        lastClockIn = null;
      }

      if (act.event_type === "lunch_in") {
        lastLunchIn = act.event_time;
        events.push({
          id: `lunch-in-${act.id}`,
          type: "Lunch Iniciado",
          ...base,
      ...
      if (act.event_type === "lunch_out") {
        events.push({
          id: `lunch-out-${act.id}`,
          type: "Lunch Terminado",
          ...base,
          duration: diffTime(lastLunchIn, act.event_time),
        });
        lastLunchIn = null;
      }

      if (act.event_type === "break_in") {
        lastBreakIn = act.event_time;
        events.push({
          id: `break-in-${act.id}`,
          type: "Break Iniciado",
          ...base,
      ...
      if (act.event_type === "break_out") {
        events.push({
          id: `break-out-${act.id}`,
          type: "Break Terminado",
          ...base,
          duration: diffTime(lastBreakIn, act.event_time),
        });
        lastBreakIn = null;
      }
    });
